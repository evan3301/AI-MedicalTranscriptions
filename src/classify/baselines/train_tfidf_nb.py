# src/classify/baselines/train_tfidf_nb.py
...
def train_nb(*, train_csv: str | Path, outdir: str | Path, val_csv: str | Path | None = None) -> Dict[str, Any]:
    outdir = Path(outdir)
    ...
    metrics: Dict[str, Any] = {"model": "nb", "train_rows": len(y_train)}
...
def main() -> None:
    import sys
    if len(sys.argv) < 3:
        print("Usage: train_tfidf_nb.py <train_csv> <outdir> [val_csv]")
        raise SystemExit(1)
    ...
    m = train_nb(train_csv=train_csv, outdir=outdir, val_csv=val_csv)
